// frontend/src/dashboard/seguimiento_cotizaciones/SeguimientoCotizaciones.jsx
...
import { getProbNombre, getProbColor, PROB_STATE_COLORS } from "@/components/ui/colors";
...
const fetchCotizacionesAprobacion = async ({ queryKey }) => {
  ...
  // 🔹 Los params externos pisan a los defaults
  const finalParams = {
    ...params,
  };
...
export default function SeguimientoCotizaciones() {
  ...
  const [currentFilters, setCurrentFilters] = useState({
    ...
    estado: "2,7",                     // todos los estados
    area: "%",                        // todas las áreas
    envio: "3",                       // todos los envíos
  ...
  const {
    ...
    queryKey: ["seguimiento-cotizaciones", currentFilters],
      ...
      <div className="flex-1 flex flex-col py-[clamp(8px,2vw,24px)] px-[clamp(8px,2vw,24px)]">
        ...
        >
          <div className="flex-1 min-w-0">
            ...
            >
              Control y avance del proceso de <span className="font-semibold text-blue-600">cotizaciones</span>.
            </motion.p>
          </div>
        ...
        <div className="w-full mb-4">
          <FilterCard
            dashboard="seguimiento-cotizaciones"
            className="w-full"
            compact
            ...
            onProcess={async (filters, event) => {
              ...
              try {
                ...
                let estadoParam = filters.estado;

                if (!estadoParam) {
                  estadoParam = "2,7"; // 👈 default
                }
        ...
        ">
          {[
            { label: "Bajo", color: PROB_STATE_COLORS["0"] },
            { label: "Media", color: PROB_STATE_COLORS["1"] },
            { label: "Alta", color: PROB_STATE_COLORS["2"] },
            { label: "Muy Alta", color: PROB_STATE_COLORS["3"] },
          ].map(({ label, color }) => (
        ...
        {logisticaSeleccionada && (
          <AprobacionCotizacionModal
            ...
            cotizacion={logisticaSeleccionada}
            modo="S"
            tipo="V"
            dashboard="S"
          />
