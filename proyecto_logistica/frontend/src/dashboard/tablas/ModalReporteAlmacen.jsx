...
const COLUMNAS = [
  ...
  { label: "Responsable", render: (r) => r.res || "—" },
  { label: "Teléfono",    render: (r) => r.tel || "—" },
  { label: "Dirección",   render: (r) => r.dir || "—" },
  {
    label: "Operativo",
...
export default function ModalReporteAlmacen({ isOpen, onClose, filtros, registros }) {
  ...
  const handleDescargar = async () => {
    ...
    setDownloading(true);
    try {
      const response = await api.get("/cotizaciones/almacenes/reporte-excel/", {
      ...
      link.href = url;
      link.setAttribute("download", `Reporte_Almacenes_${Date.now()}.xlsx`);
