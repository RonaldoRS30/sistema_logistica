...
import { FilePlus, FileSpreadsheet, Loader2, Building2, Calendar, Search, RefreshCw } from "lucide-react";
...
export default function Almacenes() {
  ...
  // Filtros
  const [estado, setEstado] = useState("1"); // Activos por defecto
  ...
  const handleProcesar = async (overrideParams = null) => {
    ...
    try {
      ...
      const res = await api.get("/cotizaciones/almacenes/", {
        params: paramsToUse,
      });
    ...
    >
      ...
      <div className="bg-slate-900 text-white flex items-center gap-3 px-6 py-4 rounded-t-2xl shadow-xl border-b border-slate-800">
        ...
        <div className="flex flex-col">
            <span className="text-sm font-black tracking-widest uppercase">Catálogo de Almacenes</span>
            <span className="text-blue-400 text-[10px] font-bold tracking-tighter uppercase opacity-70">Depósitos y Centros de Acopio</span>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 shadow-2xl overflow-hidden flex flex-col rounded-b-2xl">
          ...
          <table className="w-full text-[11px] border-collapse">
            <thead className="sticky top-0 bg-[#F2F5F8] z-10">
              <tr className="divide-x divide-slate-100 border-b border-slate-200 shadow-sm">
                ...
                <th className="px-6 py-4 text-blue-600 font-black uppercase tracking-widest text-left">Cod</th>
                <th className="px-6 py-4 text-blue-600 font-black uppercase tracking-widest text-left">Denominación del Almacén</th>
                <th className="px-6 py-4 text-blue-600 font-black uppercase tracking-widest text-left">Gst. Responsable</th>
                <th className="px-5 py-4 text-blue-600 font-black uppercase tracking-widest text-center">Teléfono</th>
                <th className="px-6 py-4 text-blue-600 font-black uppercase tracking-widest text-left">Dirección Física</th>
                <th className="px-6 py-4 text-blue-600 font-black uppercase tracking-widest text-center w-24">Operativo</th>
              </tr>
            </thead>
              ...
              {registrosPaginados.length > 0 ? registrosPaginados.map((r, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-slate-500 font-bold uppercase tracking-tight text-[10px]">{r.res || "—"}</td>
                    <td className="px-5 py-4 text-slate-700 font-black text-center tracking-tighter font-mono">{r.tel || "—"}</td>
                    <td className="px-6 py-4 text-slate-500 text-[10px] leading-tight font-medium uppercase">{r.dir || "—"}</td>
                    ...
                    <td colSpan={7} className="py-24 text-center text-slate-400 font-black uppercase tracking-widest text-[10px] bg-slate-50/20">
                        {loading ? (
                            <div className="flex flex-col items-center gap-3">
                                ...
                                <span>Sincronizando Almacenes...</span>
                            </div>
                        ) : "No existen registros registrados"}
                    </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="bg-slate-100 px-6 py-4 flex items-center justify-between text-[11px] font-black text-slate-600 border-t border-slate-200 rounded-b-2xl shadow-inner">
           <div className="flex items-center gap-3">
                ...
                <span className="tracking-widest uppercase text-slate-400">Total de Almacenes</span>
           </div>
