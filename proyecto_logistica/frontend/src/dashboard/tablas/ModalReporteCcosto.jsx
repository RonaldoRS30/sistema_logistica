...
const COLUMNAS = [
  ...
  { label: "Nombre del Centro de Costo", render: (r) => <span className="uppercase">{r.nom}</span> },
  {
    label: "Activo",
...
export default function ModalReporteCcosto({ isOpen, onClose, filtros, registros }) {
  ...
  const handleDescargar = async () => {
    ...
    setDownloading(true);
    try {
      const response = await api.get("/cotizaciones/centros-costo/reporte-excel/", {
        params: {
      ...
      link.href = url;
      link.setAttribute("download", `Reporte_Centros_Costo_${Date.now()}.xlsx`);
  ...
  return (
    <ModalReporteTabla
      isOpen={isOpen}
      onClose={onClose}
      titulo="REPORTE DE CENTROS DE COSTO"
      registros={registros || []}
      columnas={COLUMNAS}
