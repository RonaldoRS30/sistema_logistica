...
import { FilePlus, FileSpreadsheet, Loader2, PackageSearch, Calendar, Search, RefreshCw } from "lucide-react";
...
export default function Productos() {
  ...
  const handleProcesar = async (overrideParams = null) => {
    setLoading(true);
    try {
      const res = await api.get("/cotizaciones/productos/", {
  ...
  const handleLimpiar = () => {
    ...
    setValorBusqueda("");
    setTimeout(() => handleProcesar({ activo: "1", campo: "Nombre", valor: "" }), 50);
  };
    ...
    >
      ...
      <div className="bg-slate-900 text-white flex items-center gap-3 px-6 py-4 rounded-t-2xl shadow-xl border-b border-slate-800">
        ...
        <div className="flex flex-col">
            <span className="text-sm font-black tracking-widest uppercase">Maestro de Productos</span>
            <span className="text-blue-400 text-[10px] font-bold tracking-tighter uppercase opacity-70">Inventario y Stock Logístico</span>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 shadow-2xl overflow-hidden flex flex-col rounded-b-2xl">
        ...
        <div className="p-4 bg-slate-50/80 border-b border-slate-200 flex flex-wrap items-center gap-4">
          ...
          <div className="flex flex-col gap-1 w-32">
             <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Campo</label>
             <select
                value={campoBusqueda}
                onChange={(e) => setCampoBusqueda(e.target.value)}
                className="h-11 border border-slate-200 rounded-xl px-4 text-xs font-bold bg-white text-slate-700 focus:ring-4 focus:ring-blue-100 outline-none shadow-sm transition-all cursor-pointer"
             >
          ...
          <div className="flex flex-col gap-1 flex-1 min-w-[200px] max-w-md">
             <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Valor de Búsqueda</label>
          ...
          <table className="w-full text-[11px] border-collapse">
            <thead className="sticky top-0 bg-[#F2F5F8] z-10">
              <tr className="divide-x divide-slate-100 border-b border-slate-200 shadow-sm">
                ...
                <th className="px-6 py-4 text-blue-600 font-black uppercase tracking-widest text-left">Código / SKu</th>
                <th className="px-6 py-4 text-blue-600 font-black uppercase tracking-widest text-left">Descripción del Artículo</th>
                <th className="px-5 py-4 text-blue-600 font-black uppercase tracking-widest text-center">Unidad</th>
                <th className="px-6 py-4 text-blue-600 font-black uppercase tracking-widest text-left">Marca</th>
                <th className="px-5 py-4 text-blue-600 font-black uppercase tracking-widest text-center">Stock</th>
              ...
              {registrosPaginados.length > 0 ? registrosPaginados.map((r, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-5 py-4 text-slate-500 font-bold text-center uppercase tracking-tighter">{r.um || "—"}</td>
                    <td className="px-6 py-4 text-slate-500 font-bold uppercase tracking-tight text-[10px]">{r.det || "—"}</td>
                    <td className="px-5 py-4 text-slate-700 font-black text-center tracking-tighter">{r.can || "0"}</td>
                ...
                <tr>
                    <td colSpan={7} className="py-24 text-center text-slate-400 font-black uppercase tracking-widest text-[10px] bg-slate-50/20">
                        {loading ? (
        ...
        <div className="bg-slate-100 px-6 py-4 flex items-center justify-between text-[11px] font-black text-slate-600 border-t border-slate-200 rounded-b-2xl shadow-inner">
           <div className="flex items-center gap-3">
                ...
                <span className="tracking-widest uppercase text-slate-400">Total de Artículos</span>
           </div>
      ...
      <ModalReporteProducto
        isOpen={reporteModalOpen}
        onClose={() => setReporteModalOpen(false)}
        filtros={{ campo: campoBusqueda, valor: valorBusqueda, estado }}
        registros={registros}
      />
    </motion.div>
  );
