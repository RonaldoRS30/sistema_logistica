...
import { FilePlus, FileSpreadsheet, Loader2, FileText, Search, RefreshCw, Calendar } from "lucide-react";
import ModalDocAlmacen from "./ModalDocAlmacen";
...
export default function DocumentosAlmacen() {
  ...
  const handleProcesar = async (overrideParams = null) => {
    setLoading(true);
    try {
      const params = overrideParams ?? { activo: estado, q: q.trim() };
      const res = await api.get("/cotizaciones/doc-almacen/", { params });
      const data = Array.isArray(res.data) ? res.data : (res.data?.results || []);
      setRegistros(data);
      setPage(1);
    } catch (e) {
      console.error("Error cargando documentos de almacén:", e);
      setRegistros([]);
    } finally {
      setLoading(false);
    }
  ...
  useEffect(() => { handleProcesar(); }, [estado]);
    ...
    >
      ...
      <div className="bg-slate-900 text-white flex items-center gap-3 px-6 py-4 rounded-t-2xl shadow-xl border-b border-slate-800">
        ...
        <div className="flex flex-col">
          <span className="text-sm font-black tracking-widest uppercase">Documentos de Almacén</span>
          <span className="text-blue-400 text-[10px] font-bold tracking-tighter uppercase opacity-70">
            Tipos de Movimiento y Documentos
          </span>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 shadow-2xl overflow-hidden flex flex-col rounded-b-2xl">
          ...
          <table className="w-full text-[11px] border-collapse">
            <thead className="sticky top-0 bg-[#F2F5F8] z-10">
              <tr className="divide-x divide-slate-100 border-b border-slate-200 shadow-sm">
                ...
                <th className="px-6 py-4 text-blue-600 font-black uppercase tracking-widest text-left w-24">Código</th>
                <th className="px-6 py-4 text-blue-600 font-black uppercase tracking-widest text-left">Nombre / Documento</th>
                <th className="px-6 py-4 text-blue-600 font-black uppercase tracking-widest text-center w-28">Operativo</th>
              </tr>
            </thead>
                ...
                ? registrosPaginados.map((r, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-6 py-4 text-slate-800 font-black tracking-tight text-[13px] font-mono">
                          {r.cod}
                        </td>
                        <td className="px-6 py-4 text-slate-600 font-bold tracking-tight text-[11px] leading-tight">
                          {r.nom || "—"}
                        </td>
        ...
        <div className="bg-slate-100 px-6 py-4 flex items-center justify-between text-[11px] font-black text-slate-600 border-t border-slate-200 rounded-b-2xl shadow-inner">
          <div className="flex items-center gap-3">
            ...
            <span className="tracking-widest uppercase text-slate-400">Total de Documentos</span>
          </div>
