...
import { FilePlus, FileSpreadsheet, Loader2, Users, Calendar, Search, RefreshCw } from "lucide-react";
...
export default function Proveedores() {
  ...
  // Filtros
  const [estado, setEstado] = useState("1"); // Por defecto Activos
  ...
  const handleProcesar = async (overrideParams = null) => {
    setLoading(true);
    try {
      const paramsToUse = overrideParams || { activo: estado, q: q.trim() };
    ...
    >
      ...
      <div className="bg-slate-900 text-white flex items-center gap-3 px-6 py-4 rounded-t-2xl shadow-xl border-b border-slate-800">
        ...
        <div className="flex flex-col">
            <span className="text-sm font-black tracking-widest uppercase">Catálogo de Proveedores</span>
            <span className="text-blue-400 text-[10px] font-bold tracking-tighter uppercase opacity-70">Entidades y Terceros</span>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200 shadow-2xl overflow-hidden flex flex-col rounded-b-2xl">
        ...
        <div className="p-4 bg-slate-50/80 border-b border-slate-200 flex flex-wrap items-center gap-4">
          ...
          <div className="flex flex-col gap-1 flex-1 min-w-[200px] max-w-md">
             <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Búsqueda General</label>
          ...
          <table className="w-full text-[11px] border-collapse">
            <thead className="sticky top-0 bg-[#F2F5F8] z-10">
              <tr className="divide-x divide-slate-100 border-b border-slate-200 shadow-sm">
                ...
                <th className="px-6 py-4 text-blue-600 font-black uppercase tracking-widest text-left">Nombre / Razón Social</th>
                <th className="px-5 py-4 text-blue-600 font-black uppercase tracking-widest text-center">Iniciales</th>
                <th className="px-5 py-4 text-blue-600 font-black uppercase tracking-widest text-center">RUC</th>
                <th className="px-5 py-4 text-blue-600 font-black uppercase tracking-widest text-center">Tipo</th>
                <th className="px-6 py-4 text-blue-600 font-black uppercase tracking-widest text-left">Actividad</th>
              ...
              {registrosPaginados.length > 0 ? registrosPaginados.map((r, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-slate-800 font-black tracking-tight text-[12px]">{r.codigo}</td>
                    <td className="px-6 py-4 text-slate-600 font-bold uppercase tracking-tight text-[11px] leading-tight">{r.nombre}</td>
                    <td className="px-5 py-4 text-slate-500 font-bold text-center uppercase tracking-tighter">{r.iniciales || "—"}</td>
                    <td className="px-5 py-4 text-slate-700 font-black text-center tracking-tighter font-mono">{r.ruc || "—"}</td>
                    <td className="px-5 py-4 text-slate-500 font-bold text-center">{r.tipo || "—"}</td>
                    <td className="px-6 py-4 text-slate-500 text-[10px] leading-tight font-medium uppercase">{r.pro || "—"}</td>
                ...
                <tr>
                    <td colSpan={8} className="py-24 text-center text-slate-400 font-black uppercase tracking-widest text-[10px] bg-slate-50/20">
                        {loading ? (
                            <div className="flex flex-col items-center gap-3">
                                <Loader2 size={30} className="animate-spin text-blue-500" />
                                <span>Sincronizando Proveedores...</span>
                            </div>
        ...
        <div className="bg-slate-100 px-6 py-4 flex items-center justify-between text-[11px] font-black text-slate-600 border-t border-slate-200 rounded-b-2xl shadow-inner">
           <div className="flex items-center gap-3">
                ...
                <span className="tracking-widest uppercase text-slate-400">Registro(s) de Proveedores</span>
           </div>
