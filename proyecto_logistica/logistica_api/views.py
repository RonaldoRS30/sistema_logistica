...
# USUARIO #
#=========#
from logistica_api.models import SegUsuario
from rest_framework_simplejwt.tokens import RefreshToken
from django.contrib.auth.hashers import check_password
from logistica_api.serializers import SegUsuarioSerializer
from rest_framework_simplejwt.authentication import JWTAuthentication
...
def usuarios_activos(request):
    ...
    # Un solo query para todas las áreas
    areas_dict = {
        str(a.codigo): a.nombre
        for a in VcTabAreas.objects.all()
    }

    data = [
        {
            "usuario_usu":   u.usuario_usu,
            "nomb_cort_usu": u.nomb_cort_usu or "",
            "nom":           u.nom           or "",
            "ape":           u.ape           or "",
            "area":          u.area,
            "area_nombre":   areas_dict.get(str(u.area), "-"),
        }
        for u in usuarios
    ]
...
def logistica_areas_view(request):
    ...
    areas = VcTabAreas.objects.filter(activo="1")
